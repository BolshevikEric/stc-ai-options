...
- **AI 选项生成**:提取最新一条 AI 发言发送给 AI,生成选项卡片;点击填入输入框,「换一批」重新生成;AI 回复结束后自动生成(可关)
  - 可配置:生成数量、提取正则(可选,留空发送原文)、生成指令模板(占位符 `{{count}}` / `{{content}}` / `{{direction}}`,另兼容酒馆全部原生占位符)
- **点击填充方式**:默认覆盖输入框;可在设置中开启「追加选项内容」,把选项追加到已有内容末尾(用换行分隔)
- **生成方向**:「生成选项 / 换一批」右侧的「生成方向」按钮,点击展开输入行,输入本次生成选项的方向并确认后**立即按方向重新生成**;方向在会话内沿用(「换一批」与自动生成同样生效),留空确认即清除;模板中写了 `{{direction}}` 占位符则就地替换,未写则自动追加到指令末尾
- **世界书**:勾选哪些条目,生成选项时这些设定注入到正文末尾;书级复选框三态:全选 ✓ / 部分选中 —;列表默认收起,点标题右侧展开图标才加载并列出
...
3. (可选)点「生成方向」输入本次生成方向,确认后选项会朝该方向生成;留空确认即清除方向
4. 点击选项填入输入框,可编辑后发送(默认覆盖;开启「追加选项内容」后追加到已有内容末尾)
5. (可选)在设置里点世界书标题右侧展开图标,再勾选需要的世界书条目
...
|---|---|
| 启用选项框 | 总开关 |
| 追加选项内容 | 关闭:点击选项覆盖输入框;开启:追加到已有内容末尾(换行分隔) |
| AI 回复后自动生成 | 关闭后仅手动触发 |
| 生成数量 | 每次 AI 生成几个选项 |
| 提取正则 | 可选,留空发送原文;支持 `/pattern/flags`,有捕获组取第 1 组 |
| 生成指令模板 | `{{count}}` 为数量,`{{content}}` 为最新剧情,`{{direction}}` 为生成方向(未设置时替换为空);支持酒馆全部原生占位符(`{{char}}`/`{{user}}`、`<char>`/`<user>`、`{{time}}` 等) |
| 世界书 | 默认收起,点标题右侧展开图标才列出;勾选要附带给 AI 的条目;条目内同样支持原生占位符;书级复选框:全选 ✓ / 部分选中 — |
