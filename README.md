...
- **AI 选项生成**:提取最新一条 AI 发言发送给 AI,生成选项卡片;点击填入输入框,「换一批」重新生成;AI 回复结束后自动生成(可关)
  - 可配置:生成数量、提取正则(可选,留空发送原文)、生成指令模板(占位符 `{{count}}` / `{{content}}`,另兼容酒馆全部原生占位符)
...
| 生成指令模板 | `{{count}}` 为数量,`{{content}}` 为最新剧情;支持酒馆全部原生占位符(`{{char}}`/`{{user}}`、`<char>`/`<user>`、`{{time}}` 等) |
| 世界书 | 勾选要附带给 AI 的条目;条目内同样支持原生占位符;书级复选框:全选 ✓ / 部分选中 — |
