...
- **AI 选项生成**:提取最新一条 AI 发言(自动跳过系统消息与 HTML 提示)发送给 AI,生成下一步行动/回复选项,以编号卡片列表渲染在聊天末尾
  ...
  - 可配置:生成数量、提取正则(可选,按正则抽取匹配内容,留空发送原文)、生成指令模板(占位符 `{{count}}` / `{{worldinfo}}` / `{{content}}`)
  - 解析健壮性:优先解析 JSON 数组,失败后自动降级为按行解析
- **世界书**:在设置面板勾选哪些世界书的哪些条目,生成选项时这些设定会附加进提示词,让选项贴合世界观;不影响酒馆正常聊天的世界书触发
...
| 生成指令模板 | 发给 AI 的指令,`{{count}}` 为数量,`{{worldinfo}}` 为世界书设定,`{{content}}` 为最新剧情 |
