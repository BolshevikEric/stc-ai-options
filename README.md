...
- **AI 选项生成**:提取最新一条 AI 发言发送给 AI,生成选项卡片;点击填入输入框(不自动发送),「换一批」重新生成;AI 回复结束后自动生成(可关)
  - 可配置:生成数量、提取正则(可选,留空发送原文)、生成指令模板(占位符 `{{count}}` / `{{content}}`)
- **世界书**:勾选哪些条目,生成选项时这些设定注入到正文末尾;不影响酒馆正常聊天的世界书触发;书级复选框三态:全选 ✓ / 部分选中 —
...
| 生成指令模板 | `{{count}}` 为数量,`{{content}}` 为最新剧情 |
| 世界书 | 勾选要附带给 AI 的条目;书级复选框:全选 ✓ / 部分选中 — |
